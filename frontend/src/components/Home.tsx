...
function Home() {
  ...
  const tmp_users = [
    {id: "111111", name: "テスト男", icon_url: "http://localhost:3000/logo192.png"},
    {id: "222222", name: "テスト女", icon_url: "http://localhost:3000/logo192.png"},
    {id: "333333", name: "テスト爺", icon_url: "http://localhost:3000/logo192.png"},
    {id: "444444", name: "テスト婆", icon_url: "http://localhost:3000/logo192.png"},
  ]
  ...
  return <>
    <Modal show={is_searching} size="lg" onHide={() => setIsSearching(false)}>
      <Modal.Body>
        <div className='container'>
          <div className='d-flex flex-wrap mb-2'>
            ...
            <input type='text' className='px-3 py-2 m-0 col-9 border border-gray rounded-pill h3'/>
          </div>
          <div className='d-flex mb-2'>
            <div className='col-9 d-flex ml-auto px-2 py-2 lh-auto border border-gray rounded'>
              {
                trending_technologies.map(technology => {
                  return <button className='btn btn-link p-0 mx-2'>#{technology}</button>
                })
              }
            </div>
          </div>
              ...
              tmp_users.map(user => {
                return <div className='col-6 p-0'>
                  <div className='m-2 p-1 border border-gray rounded d-flex'>
                    <img className='border border-dark rounded-circle m-1' src={user.icon_url} width={50}/>
      ...
      <Modal.Footer>
	  		<button className='btn btn-secondary' onClick={() => setIsSearching(false)}>閉じる</button>
      </Modal.Footer>
    </Modal>
      ...
      <div className="d-flex mt-2">
        ...
        <div className="col-4 mx-5">
          <div className="sticky-top">
            <button className="btn btn-light border-dark w-100" onClick={() => setIsSearching(true)}>検索</button>
          </div>
          <div className="m-2">
            {
