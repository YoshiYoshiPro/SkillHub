...
function Profile() {
  ...
  const [suggested_interest_tecs, setSuggestedInterestTecs] = useState(
    [] as { id: number; name: string }[]
  );
  const [suggested_expertise_tecs, setSuggestedExpertiseTecs] = useState(
    [] as { id: number; name: string }[]
  );
  const [suggested_experience_tecs, setSuggestedExperienceTecs] = useState(
    [] as { id: number; name: string }[]
  );
  ...
  const [expertises, setExpertises] = useState(
    [] as { id: number; name: string; years: number }[]
  );
  ...
  const [edited_interests, setEditedInterests] = useState(
    [] as { id: number; name: string }[]
  );
  const [edited_expertises, setEditedExpertises] = useState(
    [] as { id: number; name: string; years: number }[]
  );
  const [edited_experiences, setEditedExperiences] = useState(
    [] as { id: number; name: string; years: number }[]
  );
  ...
  const interestTecSubstringChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      setInterestTecSubstring(e.target.value.trim());
      axios
        .get(
          "http://localhost:8000/get-suggested-tecs/" + e.target.value.trim()
        )
        .then((res) => {
  ...
  const expertiseTecSubstringChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const experienceTecSubstringChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const editedNameChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      setEditedName(e.target.value.trim());
    },
    []
  );
  const editedMailChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      setEditedMail(e.target.value.trim());
    },
    []
  );
  const editedCommentChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      setEditedComment(e.target.value.trim());
    },
    []
  );
  const editedJoinDateChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const editedDepartmentChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  // 編集中に新たにタグを追加する処理
  const editedInterestsAdd = useCallback(
    (suggested_tec: { id: number; name: string }) => {
      setInterestTecSubstring("");
      setSuggestedInterestTecs([]);
      setEditedInterests(
        (edited_interests) => new Array(...edited_interests, suggested_tec)
      );
    },
    []
  );
  const editedExpertisesAdd = useCallback(
    (suggested_tec: { id: number; name: string }) => {
      setExpertiseTecSubstring("");
      setSuggestedExpertiseTecs([]);
      setEditedExpertises(
        (edited_expertises) =>
          new Array(...edited_expertises, { ...suggested_tec, years: 1 })
      );
    },
    []
  );
  const editedExperiencesAdd = useCallback(
    (suggested_tec: { id: number; name: string }) => {
      setExperienceTecSubstring("");
      setSuggestedExperienceTecs([]);
      setEditedExperiences(
        (edited_experiences) =>
          new Array(...edited_experiences, { ...suggested_tec, years: 1 })
      );
    },
    []
  );
  ...
  const editedInterestsErase = useCallback((target_index: number) => {
    setEditedInterests((edited_interests) =>
      edited_interests.filter((_, index) => target_index !== index)
    );
  }, []);
  const editedExpertisesErase = useCallback((target_index: number) => {
    setEditedExpertises((edited_expertises) =>
      edited_expertises.filter((_, index) => target_index !== index)
    );
  }, []);
  const editedExperiencesErase = useCallback((target_index: number) => {
    setEditedExperiences((edited_experiences) =>
      edited_experiences.filter((_, index) => target_index !== index)
    );
  }, []);

  // 編集中にタグに対する年数を変更する処理
  const editedExpertisesYearsChange = useCallback(
    (target_index: number, new_years: number) => {
      setEditedExpertises((edited_expertises) =>
  ...
  const editedExperiencesYearsChange = useCallback(
    (target_index: number, new_years: number) => {
      setEditedExperiences((edited_experiences) =>
        edited_experiences.map((experience, index) => {
          if (target_index === index)
            return { ...experience, years: new_years };
          return experience;
        })
      );
    },
  ...
  useEffect(() => {
    // プロフィール情報を取得する
    axios
      .get("http://localhost:8000/get-profile/" + user_id)
      .then((res) => {
          ...
          <div className="col-8">
            ...
            <div className="mb-5 d-flex">
              ...
              {is_editing ? (
                ...
                <p className="text-center w-75 ml-auto">
                  {new Date(join_date).toLocaleDateString("sv-SE")}
                </p>
              )}
            </div>
            <div className="mb-5 d-flex">
              ...
              {is_editing ? (
                <div className="w-75">
                  {edited_interests.map((interest, index) => {
                    return (
                      <div className="d-flex">
                        ...
                        <button
                          className="btn btn-secondary ml-3 p-1 h6"
                          onClick={() => {
                            editedInterestsErase(index);
                          }}
                        >
                          削除
                        </button>
                  ...
                  <input
                    ...
                    value={interest_tec_substring}
                    onChange={interestTecSubstringChange}
                  />
                  {suggested_interest_tecs.map((suggested_tec) => {
                    return (
                      <button
                        ...
                        onClick={() => {
                          editedInterestsAdd(suggested_tec);
                        }}
                      >
                        {suggested_tec.name}
                      </button>
              ...
              {is_editing ? (
                <div className="w-75">
                  {edited_experiences.map((experience, index) => {
                    return (
                      <div className="d-flex">
                        ...
                        <input
                          type="number"
                          className="w-25 form-control ml-2"
                          value={experience.years}
                          onChange={(e) => {
                            editedExperiencesYearsChange(
                              index,
                  ...
                  <input
                    type="text"
                    className="w-50 form-control"
                    value={experience_tec_substring}
                    onChange={experienceTecSubstringChange}
                  />
                  {suggested_experience_tecs.map((suggested_tec) => {
                    return (
                      <button
                        className="btn btn-link p-0 mx-2"
                        onClick={() => {
                          editedExperiencesAdd(suggested_tec);
                        }}
                      >
                        {suggested_tec.name}
                      </button>
              ...
              {is_editing ? (
                <div className="w-75">
                  ...
                  <input
                    type="text"
                    className="w-50 form-control"
                    value={expertise_tec_substring}
                    onChange={expertiseTecSubstringChange}
                  />
                  {suggested_expertise_tecs.map((suggested_tec) => {
                    return (
                      <button
                        className="btn btn-link p-0 mx-2"
                        onClick={() => {
                          editedExpertisesAdd(suggested_tec);
                        }}
                      >
                        {suggested_tec.name}
                      </button>
